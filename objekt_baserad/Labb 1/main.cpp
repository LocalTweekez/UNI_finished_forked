...
std:: istream& operator >>(std::istream& in, Person& p){ // overloaded operator >>
    ...
    for (int i = a; i <location.size(); i++) {
        ...
        else if(location[i] == ',' ){//making sure that the comma is removed
            continue;
        }
        else{
            city += location[i]; // the rest is the city name; ÅÄÖ istn't in the ascii table so this is the easiest way.
        }
